...
double ArithmeticParser::parse_Expression() {
	...
	if (!is_tb_Expression()&&!is_tb_Identifier()) {
...
double ArithmeticParser::get_Identifier_value(std::string x) {
	return vars[x];
}
double ArithmeticParser::parse_Term() {
	//Term=Factor {MultOp Factor}
	if (!is_tb_Term()&& !is_tb_Identifier()) {
...
double ArithmeticParser::parse_Factor() {
	double value{ 0 };
	if (!is_tb_Factor() && !is_tb_Identifier()) {
